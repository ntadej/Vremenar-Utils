...
def set_cron_item_interval(cron: CronItem, command: str, db_type: DatabaseType) -> None:
    ...
    if command in ["alerts-update", "arso-maps"]:
        ...
        if db_type == DatabaseType.Production:
            ...
            cron.minute.on(45)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    elif command == "dwd-mosmix":
        if db_type == DatabaseType.Production:
            cron.minute.on(35)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
        else:
            cron.minute.on(40)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    else:
